//! The `jinn:profile` provider (M2-K7, harness #21; contract bundle
//! `contracts/jinn-profile`, constitution 04): a profile patch as OPERATOR
//! INTENT. `patch-entry(id, merge-patch)` is applied BY THE LOADER — the
//! runtime-led amendment: validated, written back atomically (stage +
//! fsync + rename), the patched fiber restarted — and RECORDED as a typed
//! `ProfilePatched` ledger event with no fs inverse and no fiber journal
//! entry: the profile's history is the ledger's, not a fiber's
//! contribution, so disposing the editor never touches the document.
//! Authority is the caller's `entry-ids` scope, fail-closed (a bare grant
//! patches nothing; `*` only when written); every refusal is on the record.
...
impl HostProfile {
    ...
    async fn patch_entry(&self, caller: PeerId, payload: Vec<u8>) -> Result<Vec<u8>, KernelError> {
        ...
        if let Err(refused) = self.loader.update_entry(&entry, config).await {
            return Ok(self.refuse(&entry, by, fiber, &refused.message));
        }
        sync_transitions(&self.fibers, &self.ledger);
        let by_name = by
            .as_ref()
            .map_or_else(|| format!("peer:{caller}"), |by| by.0.clone());
        self.ledger.record(
            LedgerEventKind::ProfilePatched { entry, by: by_name },
            by,
            fiber,
        );
        Ok(vec![TAG_APPLIED])
