...
use crate::bindings::{self, Plugin, lifecycle};
use crate::broker::{Broker, PeerId};
...
/// activation has registered so far.
...
impl bindings::services::Host for HostState {
    async fn provide(&mut self, contract: String) -> Result<u64, bindings::types::KernelError> {
        ...
        if !self.seat.staging {
            let face: Arc<dyn crate::broker::Peer> = match &self.seat.slot {
