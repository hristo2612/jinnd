//! The standard on-disk document store: write-temp + fsync + rename, so the
//! document on disk is always whole (LAW §3; split from `store.rs` by the
//! 300-line file cap, R10).
...
impl DocumentStore for FileStore {
    fn save<'a>(&'a self, document: &'a Document) -> KernelFuture<'a, ()> {
        Box::pin(FileStore::save(self, document))
    }
}
